...
parseLine24QQ :: String -> Q Exp
parseLine24QQ s =
  case parse noteLine24 "" s of
    Left  err    -> fail (show err)
    Right x      -> [e| x |]

line24 :: QuasiQuoter
line24 = QuasiQuoter {
    quoteExp  = parseLine24QQ
...
noteLines24 :: GenParser Char st [[(Note24, Rational)]]
noteLines24 = do
  ls <- noteLine24 `sepBy` many (oneOf " \n;")
  eof
  return ls

noteLine24 :: GenParser Char st [(Note24, Rational)]
noteLine24 = do
  _     <- spaces
  ndurs <- parseNoteDur `sepBy` many (oneOf " ,")
  return ndurs
  where parseNoteDur :: GenParser Char st (Note24, Rational)
        parseNoteDur = do
          note <- noteName
          _    <- spaces
          dur  <- duration
...
noteName :: GenParser Char st Note24
noteName = do
  ...
  let note = case note_head of
                ...
                'G' -> G
                otherwise -> undefined
  return (PitchClass24 note as, read [n] :: Int)
